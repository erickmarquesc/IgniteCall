...
export function ConfirmStep({ schedulingDate, onCancelConfirmation }: IConfirmStepProps) {
  const {
    register,
    handleSubmit,
    formState: { isSubmitting, errors } } = useForm<ConfirmFormData>({
  ...
  return (
    <ConfirmForm as="form" onSubmit={handleSubmit(handleConfirmScheduling)}>
      ...
      <label>
        ...
        <TextInput type="email" placeholder="johnGalt@example.com" {...register('email')} />
        {errors.email && <FormError size='sm'>{errors.email.message}</FormError>}

      </label>
      <label>
        <Text size="sm">Observações</Text>
        <TextArea placeholder="Escreva uma observação!" {...register('observations')} />
      </label>
